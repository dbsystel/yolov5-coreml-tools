...
class BoundingBoxes:
    ...
    def addBoundingBoxes(self, bbs):
        self._boundingBoxes.extend(bbs._boundingBoxes)
    ...
    def drawAllBoundingBoxes(self, image, imageName):
        ...
        for bb in bbxes:
            if bb.getBBType() == BBType.GroundTruth:  # if ground truth
                image = add_bb_into_image(image, bb, color=(0, 255, 0))#, label = bb._classId)  # green
            else:  # if detection
                image = add_bb_into_image(image, bb, color=(255, 0, 0)) #, label = bb._classId)  # red
        return image
